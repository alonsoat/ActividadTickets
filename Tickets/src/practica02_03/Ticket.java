	...
	public void insertar(Connection conexion) {
		...
		try {
			...
			preparedStatament = conexion.prepareStatement(sql,
					preparedStatament.RETURN_GENERATED_KEYS);
	...
	public void actualizar(Connection conexion, boolean admin) {
		...
		try {
			...
			if (this.estado.equals("Tancat") && admin == true) {
				...
				/*
				 * Probar las 2 formas de cambiar la fecha. Desde codigo en el
				 * propio metodo, mezclando 2 conexiones. Llamando al metodo
				 * cambiarFecha , que se le pasa una conexion.
				 * 
				...
				PreparedStatement preparedStatament2 = conexion
						.prepareStatement(sqlFecha);
